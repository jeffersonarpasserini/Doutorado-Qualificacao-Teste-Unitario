...
def treina_modelo(model_cnn, model_red, model_class, number_comp, df_modelo):
    ...
    features = feature_model_extract(model_cnn, df_modelo)
    features_redution = dimensinality_reduction(model_red, number_comp, features)
    clf_model = classification_model(features_redution, labels, model_class)

    return clf_model, features
...
for img_name in filenames: #lista de arquivos a serem testados
    ...
    clf, img_features_dataset = treina_modelo(model_type, model_reduction, model_classifier, number_reduce_components, df)

    #labels array
    img_label = df_teste["category"].replace({'clear': 1, 'non-clear': 0}).to_numpy().astype(int)

    img_features_extract = feature_model_extract(model_type, df_teste)
    
    img_features = np.vstack((img_features_dataset,img_features_extract))
    
    img_features_reduction = dimensinality_reduction(model_reduction, number_reduce_components, img_features)
